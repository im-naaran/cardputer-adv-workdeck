...
- 顶栏保持原样；内容区按现有中文字体排版，底部显示当前按键提示或操作结果。列表同时显示约 4 行，焦点移动时滚动，绘制前按真实字号计算可见区域。
- 首页及子页列表用现有方向映射移动，Enter 进入/确认，Tab 循环移动焦点作为关闭方向映射时的可用路径；Backspace 返回上级。
- 亮度页左右逐档调整并即时保存，边界停留，不循环；Tab 也可依次选择五档并即时保存，失败后 Enter 重试保存当前档位。
- 字段编辑采用单行尾部追加及退格，不新增复杂光标编辑器。Enter 将字段写入内存草稿并回表单，Tab 同样提交字段后移到下一字段；此时不写 Flash。
- 字段进入编辑时保留原值。长字段按 UTF-8 字符边界折行或滚动展示；显示尾部输入位置，并以专用查看状态分页查看完整值。查看状态使用方向键/Tab 翻页、Enter 返回，避免和字符输入混用。
...
- 设置页先于 BLE 离线遮罩分支绘制和处理输入；其他模块维持原有离线行为。脚本全局反馈沿用现有短暂提示，消失后恢复设置页底部内容。
...
- Codex UI 只接受 1～60 的十进制整数字符串，再乘以 60 传入现有服务。已有非整分钟值按秒如实显示，进入页面不取整保存；输入框留空等待用户明确设置。
...
3. 亮度：先应用档位保证即时反馈，再保存并回读。失败时保持实际亮度和未保存状态，用户可 Enter 重试；相同档位仍可重试失败保存，但磁盘已是相同值时跳过写入。
