...
struct App {
  ...
  NavigationService navigation;InputRouter input;KeyPressTracker keys;CodexPage codexPage;ScriptsPage scriptsPage;
  ...
  ScriptsController scripts{ids,[&](const auto& id,const auto& b,uint32_t t){return send(id,b,t);},[&](const auto& id){queue.cancelPending(id);}};
  App() {
    ...
    messages.registerHandler(protocol::kHelloAction,[&](const Message& m){
      ...
      scripts.onSessionReady(session.computerId(),session.supports(protocol::kActionsListAction),session.supports(protocol::kScriptsExecuteAction),session.supports(protocol::kShortcutExecuteAction),now);
    });
    ...
    for(const auto* action:{protocol::kActionsListAction,protocol::kScriptsExecuteAction,protocol::kShortcutExecuteAction})
      messages.registerHandler(action,[&](const auto& m){scripts.onMessage(m,now);});
  }
  void hello(bool scriptSupport=true,std::string pc="pc") {
    Message m;m.actionId=protocol::kHelloAction;m.resultCode="OK";m.protocolVersion=1;m.computerId=pc;m.computerName="PC";
    m.capabilities={protocol::kTimeReadAction,protocol::kCodexUsageAction};
    if(scriptSupport)m.capabilities.insert(m.capabilities.end(),{protocol::kActionsListAction,protocol::kScriptsExecuteAction,protocol::kShortcutExecuteAction});
    messages.route(m);
  }
  void key(KeyEvent event) {
    ...
    if(navigation.current()==Module::kCodex) {
      ...
      if(r.action==InputAction::kConfirm)scripts.confirm(now);
      if(r.action==InputAction::kDirection&&r.event.key==Key::kDown)scripts.moveSelection(1,now);
      if(r.action==InputAction::kDirection&&r.event.key==Key::kUp)scripts.moveSelection(-1,now);
    }
  }
  ...
  void page(bool success=true) {
    Message m;m.actionId=protocol::kActionsListAction;m.execId=scripts.directoryExecId();m.resultCode=success?"OK":"ERROR";
    m.total=2;m.scripts={{"script.first","第一项","g","g"},{"script.second","第二项","g",""}};messages.route(m);
  }
  void finishScript() {
    Message m;m.actionId=sent.back().actionId;m.execId=scripts.executionExecId();m.resultCode="OK";
    m.executedActionId="script.first";m.executedName="第一项";m.hasExitCode=true;messages.route(m);
  }
  void tick(uint32_t t) {
    now=t;const auto d=scripts.state().directory;const auto e=scripts.state().execution;bool feedback=!scripts.state().feedback.empty();
    scripts.tick(now);dirty=d!=scripts.state().directory||e!=scripts.state().execution||feedback!=!scripts.state().feedback.empty();
...
void four_pages_keys_and_feedback_expiration() {
  ...
  for(int m=0;m<4;++m) {
    ...
    app.key({Key::kCharacter,false,'g',true});TEST_ASSERT_EQUAL(before+1,app.sent.size());
    TEST_ASSERT_TRUE(app.scripts.state().feedback.find("请稍候")!=std::string::npos);
    ...
    TEST_ASSERT_FALSE(app.scripts.state().feedback.empty());
    app.tick(finished+2999);TEST_ASSERT_FALSE(app.dirty);TEST_ASSERT_FALSE(app.scripts.state().feedback.empty());
    app.tick(finished+3000);TEST_ASSERT_TRUE(app.dirty);TEST_ASSERT_TRUE(app.scripts.state().feedback.empty());
...
void fn_enter_and_mapping_config() {
  ...
  app.key({Key::kDigit2,true});app.key({Key::kCharacter,false,'.'});TEST_ASSERT_EQUAL(0,app.scripts.state().selected);
  ...
  app.input.applyConfig({});app.key({Key::kCharacter,false,'.'});TEST_ASSERT_EQUAL(1,app.scripts.state().selected);
  ...
  app.key({Key::kDigit2,true});TEST_ASSERT_EQUAL(1,app.scripts.state().selected);
}
void physical_hold_and_disconnect() {
  ...
  const auto old=app.scripts.executionExecId();app.disconnect();TEST_ASSERT_TRUE(app.scripts.state().feedback.empty());
  ...
  TEST_ASSERT_TRUE(app.scripts.state().feedback.find("不可用")!=std::string::npos);
  ...
  app.messages.route(late);TEST_ASSERT_TRUE(app.scripts.state().feedback.empty());
}
void old_hello_and_invalid_renegotiation() {
  ...
  app.hello(true,"other");TEST_ASSERT_TRUE(app.scripts.state().entries.empty());TEST_ASSERT_TRUE(app.scripts.executionExecId().empty());
  Message invalid;invalid.actionId=protocol::kHelloAction;app.messages.route(invalid);
  TEST_ASSERT_FALSE(app.session.ready());TEST_ASSERT_TRUE(app.scripts.directoryExecId().empty());
