...
BLE 回调只提交字节和连接状态，业务逻辑在主循环串行处理。消息处理后重新读取时间，避免响应记录的新时间与循环旧快照相减发生无符号下溢。电量独立采样并缓存；分钟显示任务仅在可见时间文字变化时置脏，实际绘制仍在主循环。息屏只跳过绘制和帧传输，后台处理继续。

BLE 已连接不代表业务就绪，必须接受合法 hello。重复 hello 不重复首次请求；hello 不包含本地刷新配置，不能覆盖文件周期或重新开启用户关闭的自动任务。身份变化或连接代次变化会清空旧请求、Codex 缓存与双动作目录/反馈；系统走时及自动开关保留。
...
| `kBatterySample` | 60000 ms | 注册时首次采样，之后周期读取并缓存电量/充电状态；变化才置脏 |
| `kDisplayRefresh` | 60000 ms | 亮屏 Codex 页比较可见时间文字，变化才置脏 |

任务服务提供注册、查询、启停、改周期、立即执行、重设起点和取消。恢复启用等待完整周期；立即执行暂停任务不恢复自动状态。回调代次快照支持自取消，派发期间拒绝递归执行；回调必须快速返回。

Codex、授时、动作控制器共享 `ExecIdGenerator`。Codex 和授时各自最多一个在途请求；动作每种类型最多一个目录请求，两类共用一个执行请求。首次合法 hello 按授时、Codex、当前可见动作目录顺序提交；目录和执行能力分别检查。Codex 入队成功的手动查询才重设周期；响应失败保留旧缓存和成功年龄。授时按最近成功状态处理同电脑重连，到期、失败或更换电脑时补同步。
...
`Fn+4` 进入，设置输入和绘制位于 BLE 离线拦截前。首页有“屏幕亮度”“Wi-Fi”“Codex自动刷新”“自动息屏”。列表用上下或 Tab 选择，Enter 确认，Backspace 返回；关闭 `settings.directionMapping` 后仍可用 Tab/Enter/Backspace 完成操作。

| 功能 | 操作及结果 |
| --- | --- |
| 亮度 | 左右逐档调整，边界停止；Tab 循环五档。20% / 40% / 60% / 80% / 100% 对应 51 / 102 / 153 / 204 / 255，默认第三档。调整立即应用并保存，同值不重复写入；失败保留当前亮度并显示错误，可按 Enter 重试保存 |
| 自动息屏 | 默认 10 分钟，可选永不/1/5/10/30 分钟；调整即保存，失败可按 Enter 重试，详见下方省电说明 |
...
| 配置文件 | 默认及恢复 |
| --- | --- |
| `/config/display.json` | `{"brightnessLevel":3,"autoScreenOffSeconds":600}`；旧亮度文件兼容，缺失/损坏时开机使用 60% 和 10 分钟 |
...
`application/wifi/WifiService` 独立于页面。main 只装配一个实例，SettingsController::tick 由主循环调用并驱动该实例；未来模块使用同一实例，不再单独 tick 或直接调用平台 WiFi API。配置查询不启用无线，没有业务需求时不连接、不自动重试连接、不按帧读文件；已有请求的关闭失败会进行有限收尾重试。
...
| `close(requestId)` | 仅本次持有者可取消/关闭；成功为 kOff，关闭失败为 kReleaseFailed，保留身份并自动有限重试，仍可手动重试 |
...
// kReleaseFailed：停止业务，保留 requestId；服务自动有限重试，预算耗尽后可手动重试。
...
`ActionsController` 为 script/clipboard 各保存当前页最多 8 条及分页元数据，两者共用一个执行状态；两个 `ActionListPage` 实例各展示最多 4 行，使选中项保持可见。全表首尾停止；跨页加载期间忽略方向输入且 Enter 不执行旧页；加载失败后 Enter 只重读目标页，成功后再次 Enter 才执行。切栏保留页、选中位置及在途执行，断线或身份变化清空。

Fn+2 进入脚本，Fn+3 进入剪贴板；Enter 分别执行或粘贴。hello 只初始化能力，首次进入动作页才加载该类目录；若 hello 到达时已在动作页，则立即加载当前类型。两类目录可以同时在途，切页保留独立选择和滚动，断线清理两页。协议 v2 与桌面配套更新；不支持剪贴板的执行器显示目录不可用，离线使用公共等待画面。
...
更多操作及实际结果记录见 [剪贴板与动作列表验收](../specs/20260907_clipboard_direct_paste/acceptance.md)。
...
`Fn+4 → 自动息屏` 提供“永不 / 1 / 5 / 10 / 30 分钟”，默认 10 分钟。方向键调整、Tab 循环，改变后立即应用和保存；失败保留当前生效值，Enter 可重试保存，Backspace 返回。亮度和息屏时长保存在同一个 `/config/display.json`：

```json
{"brightnessLevel":3,"autoScreenOffSeconds":600}
```

时长只接受 `0/60/300/600/1800` 秒，0 表示永不；旧版只有亮度的文件缺省 600 秒，无需重新 `uploadfs`。保存完整当前配置，修改一项不会重置另一项；息屏不把亮度 0 或临时亮灭状态写入文件。回滚到只接受亮度字段的旧固件前，应恢复备份的兼容显示配置，避免旧解析器拒绝新字段。

`ScreenPowerController` 以单调时间管理亮屏、息屏和等待唤醒键释放三个状态。物理活动包括修饰键及释放；键按住期间不息屏，释放后重新计时。56 键键盘任意键唤醒，G0 不在本期范围。首键及组合键的释放过程先于 InputRouter 被消费，不输入、不切页、不查询、不执行脚本或粘贴；全部释放后的下一次按键正常。

主循环继续推进 BLE、消息、设置保存、查询、授时、动作超时及 Wi-Fi 收尾。背光关闭后不绘制/推帧，刷新需求合并保留；唤醒先推送最新帧，再恢复用户亮度。Codex 时间显示只比较可见页脚与两行重置文字，静态页不再因分钟任务无条件重绘。

`PlatformBatteryAdapter` 是电池硬件读取的唯一入口，`BatteryService` 提供缓存。共享调度注册时读取一次，此后每 60 秒读取一次；亮屏/息屏相同，普通按键、消息和唤醒均不触发采样。延迟跨多个周期只采一次，失败等待下次周期，无效百分比显示未知。充电状态沿用原硬件接口并一并缓存，不据此切换电源策略。

Wi-Fi 首次释放失败后最多自动重试 3 次，每次距离上次清理完成至少 1000 ms；主循环延迟不补跑。成功立即停止，耗尽预算保留错误和网络信息页的手动关闭入口。手动重试不恢复预算，旧请求不能关闭新请求；扫描/认证清理/驱动关闭均确认后才报告释放完成。收尾不停止 BLE，不擦除 Flash。

初始化禁用未使用的内部 IMU、麦克风和扬声器，保留键盘共享 I²C；没有调整 CPU 频率、BLE 参数或主循环等待。禁止初始化不等于器件已断电，冷启动/热重启及实际功耗见 [验收记录](../specs/20260910_firmware_power_saving/acceptance.md)。
